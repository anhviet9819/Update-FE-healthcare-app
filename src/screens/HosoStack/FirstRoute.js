...
const dataHandler = (data) => {
    if (data) {
        data.sort((a, b) => (dateStringHandler(b.thoigiandk).getTime() - dateStringHandler(a.thoigiandk).getTime()))
...
const FirstRoute = (props) => {
    ...
    useEffect(() => {
        if (props.benhnhans.length === 0) {
            props.fetchBenhnhans()
        }
        dataBenhnhans = props.benhnhans.slice()
        dataBenhnhans.forEach(item => props.fetchHosos(zeroDay.toISOString(), currentDate.toISOString(), zeroDay.toISOString(), endOfMonth.toISOString(), false, item.benhnhanphu.id))
        console.log("789789",props.benhnhans)
        // console.log(dataBenhnhans.map(item => item.benhnhanphu.id))
        // props.fetchHosos(zeroDay.toISOString(), currentDate.toISOString(), zeroDay.toISOString(), endOfMonth.toISOString(), false, dataBenhnhans.map(item => item.benhnhanphu.id))
    }, [props.benhnhans])
